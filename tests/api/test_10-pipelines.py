...
class TestPipelines:
    ...
    def test_pipeline_upload_contents_match(self, testapp, temp_pipeline):  
        ...
        uploaded_file = Path(testapp.app.config.get("PYRSCHED_PIPELINES_BASE_PATH")) / uploaded_filename
    ...
    @pytest.mark.parametrize("filename", ["/etc/passwd", "../../bla.yaml", "C:/Programme/test.yaml", "%C3%A9%C3%A9%C3%A9", ])       
    def test_malicious_upload(self, testapp, temp_pipeline, filename):
        pipeline = temp_pipeline.make_pipeline()   
        files = [("body", str(pipeline['file'])), ]
        res = testapp.post(f"{self.endpoint}/{filename}", upload_files=files, expect_errors=True)
    ...
    def test_pipeline_replace_malicious(self, testapp, temp_pipeline, filename):
        pipeline = temp_pipeline.make_pipeline()   
        files = [("body", str(pipeline['file'])), ]
        res = testapp.put(f"{self.endpoint}/{filename}", upload_files=files, expect_errors=True)
        assert res.status_int >= 400  
    ...
    def test_pipeline_replace_missing_target(self, testapp, temp_pipeline):
        ...
        try:    
            target_file = Path(testapp.app.config.get("PYRSCHED_PIPELINES_BASE_PATH")) / uploaded_filename
            target_file.unlink()  # this would be easier in 3.8: there is an optional argument missing_ok=True which suppresses the exception
        except FileNotFoundError:
            pass

        pipeline = temp_pipeline.make_pipeline()
        files = [("body", str(pipeline['file'])), ]
        res = testapp.put(f"{self.endpoint}/{uploaded_filename}", upload_files=files, expect_errors=True)
    ...
    def test_pipeline_replace_with_other_file(self, testapp, temp_pipeline):
        ...
        files = [("body", str(other_pipeline['file'])), ]
    ...
    def test_pipeline_delete(self, testapp, temp_pipeline):
        ...
        res = testapp.delete(f"{self.endpoint}/nonexisting_pipeline.yaml", expect_errors=True)
        assert res.status_int == 404
